...
test('type incorrect nickname', async () => {
  render(<App />);
  const input = screen.queryByPlaceholderText(/nickname/i);
  fireEvent.change(input, { target: { value: '!!JeżJeży[]' } });
  fireEvent.click(screen.getByText(/Next/i));
...
test('type incorrect card number', async () => {
  render(<App />);
  const input = screen.queryByPlaceholderText(/number/i);
  fireEvent.change(input, { target: { value: 11122 } });
  fireEvent.click(screen.getByText(/Finish/i));

  const result = await screen.findByText(
    /card/i //need to fix this RegExp, findByText doesnt match multiline string in the doc
  );
  expect(result).toBeInTheDocument();
});
